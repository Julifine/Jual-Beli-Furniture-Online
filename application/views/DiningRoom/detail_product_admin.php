    ...
    <div class="container">
        <div class="row justify-content-md-center">
            <div class="col-md-10 align-center">
                <h1 class="mbr-section-title mbr-bold pb-3 mbr-fonts-style display-1">DINING ROOM</h1>
            </div>
        </div>
    </div>
</section>
  ...
  <div class="container">
        <div class="row justify-content-center">
            <div class="title col-lg-12" style="color: white">
				<span><a class="link-nav" href="javascript:toOtherPage('<?= base_url();?>')">Home</a> > <a class="link-nav" href="javascript:toOtherPage('<?= base_url();?>DiningroomCatalogue/')">Diningroom</a> > <a class="link-nav" href="javascript:toOtherPageWspace('<?=$productCategory;?>')" id="productCategoryLink"></a> > <b id="productNameLink"></b></span>
            </div>
        </div>
  </div>
            ...
            <div class="media-container-column col-lg-12" data-form-type="formoid">
				<div class="dragArea row">
					...
					<div class="col-lg-6" data-for="name" style="color: white;padding-left: 50px;">
						...
						<div class="row" style="margin-top: 40px;">
							<button onClick="toOtherPage('<?= base_url();?>DiningroomCatalogue/editProduct/<?=$productCategory;?>/<?=$productName;?>')" id="btn-add-cart" style="width: 100%;padding: 10px;border-radius: 16px;background-color: #D2C919;border: none;font-size: 24px;font-weight: bold">
...
<script>  
	...
	function deleteProduct(productName,productCategory){
		...
		var child = [];
		firebase.database().ref("products/Diningroom/"+newProdCat).once('value', function(snapshot) {
			...
			for(i = 0; i < child.length; i++){
				if(newProdName == child[i].productName){
					...
					firebase.storage().ref().child("products/" + child[i].roomCategory +"/"+ child[i].productCategory+"/"+child[i].productName+"/productImage.png").delete().then(function() {
					  window.location = "<?= base_url();?>DiningroomCatalogue/roomProduct/"+nReplaceAnd(newProdCat);
						...
						if(user.uid == childKey){
							if(childData.type == "user"){
								...
								if (productName != null){
									...
									var child = [];
									firebase.database().ref("products/Diningroom/"+newSub).once('value', function(snapshot) {
										snapshot.forEach(function(childSnapshot) {
											var childKey = childSnapshot.key;
											var childData = childSnapshot.val();
											child.push(childData);
