...
<script>
		...
		firebase.database().ref("products/Diningroom/"+newSub).once('value', function(snapshot) {
			...
			if (child != null){
				for(var i=0;i<child.length;i++){
					...
					gallery_item.id = "content-item-"+i;gallery_item.style.width = "300px"
									...
									if(childData.type == "user"){
										...
										for(var i=0;i<elements.length;i++){
											document.getElementById("content-item-"+i).onclick = function() {
												...
												window.location = '<?=base_url();?>DiningroomCatalogue/detailProduct/'+sub+"/"+this.dataset.tags;
											};
										}
									}else{
										...
										for(var i=0;i<elements.length;i++){
											document.getElementById("content-item-"+i).onclick = function() {
												//console.log(this.dataset.tags);
												window.location = '<?=base_url();?>DiningroomCatalogue/detailProductAdmin/'+sub+"/"+this.dataset.tags;
											};
										}
									}
								}
	...
	function stringSplit(kata_kata){
		...
		return toOtherPage('<?=base_url();?>DiningroomCatalogue/addProduct/'+newString);
	}
</script>