...
<script>  
	...
	function stringSplit(kata_kata,kata_kata_1){
		...
		return toOtherPage('<?=base_url();?>KitchenCatalogue/detailProductAdmin/'+newString1+"/"+newString);
	}
						...
						firebaseRef.child(productName).update({
							...
							if (error) {
							  ...
							  // Data saved successfully!
								...
								window.location = '<?= base_url();?>KitchenCatalogue/detailProductAdmin/'+nReplaceAnd(productCategory)+"/"+productName;
							}
						});
					});
				});
	...
	function loadView(){
		...
		if (productName != null){
			...
			var child = [];
			firebase.database().ref("products/Kitchen/"+newProductCategory).once('value', function(snapshot) {
